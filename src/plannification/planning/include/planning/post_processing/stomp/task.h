...
namespace stomp
{
  ...
  {
    public:
      Task(int max_iter, int window_size, double tolerance, double max_time, std::string stopping_condition) : max_iter_(max_iter), windowSize_(window_size), tolerance_(tolerance), max_time_(max_time), stopping_condition_(stopping_condition) {}
       ...
       */
      virtual std::vector<ompl::base::State*> toOMPL(const Eigen::MatrixXd& parameters) = 0;

      /**
        * @brief Free a state vector
        * @param data The state vector
      */
      virtual void freeStateVector(std::vector<ompl::base::State*> &states) const = 0;
      ...
      /**
       * @brief Called by Stomp at each cost improvement to export the results
       *
       * @param traj              The Optimized trajectory
       * @param cost              Current best cost
       * @param plan_time         Current planning time
       */
      virtual void exportShct(std::vector<ompl::base::State *>& traj, double cost, double plan_time) const = 0;

      /** \brief Termination condition */
      virtual bool stoppingCriterion() = 0;
