#include "planning/bullet/BulletClient.h"
#include "planning/bullet/PhysicsServers.h"
#include "planning/bullet/RosFiles.h"
...
void BulletClient::setAdditionalSearchPath(const std::string& path)
{
	...
	if (path != "")
	{
        ...
        if(additional_path_ != "")
            ROS_WARN("[Bullet] The previous additional path has been overwritten");
                                    ...
                                    const std::array<double, 4>& rgba_color)
{
	...
	if (shape_type >= GEOM_SPHERE)
	{
		...
		if (shape_type == GEOM_SPHERE)
		{
            if(radius > 0)
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create sphere visual shape");
		}
		...
		else if (shape_type == GEOM_CAPSULE)
		{
            if(radius > 0 && height >= 0)
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create capsule visual shape");
		}
		else if (shape_type == GEOM_CYLINDER)
		{
            if(radius > 0 && height >= 0)
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create cylinder visual shape");
		}
		else if (shape_type == GEOM_MESH)
		{
            if(file_name != "")
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create mesh visual shape");
		}
    ...
    ROS_ERROR("[Bullet] createVisualShape failed.");
	return -1;
}
                                        ...
                                        int flags)
{
	...
	if (shape_type >= GEOM_SPHERE)
	{
		...
		{   
            if(radius > 0)
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create box collision shape");
		}
		else if (shape_type == GEOM_BOX)
		{
		...
		else if (shape_type == GEOM_CAPSULE)
		{
            if(radius > 0 && height >= 0)
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create capsule collision shape");
		}
		else if (shape_type == GEOM_CYLINDER)
		{
            if(radius > 0 && height >= 0)
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create cylinder collision shape");
		}
		else if (shape_type == GEOM_MESH)
		{
			if(file_name != "")
                ...
                ROS_ERROR("[Bullet] Invalid parameters to create mesh collision shape");
		}
    ...
    ROS_ERROR("[Bullet] createCollisionShape failed. ");
	return -1;
}
                                  ...
                                  int flags)
{
	...
	ROS_ERROR("[Bullet] createMultiBody failed.");
	return -1;
}
...
int BulletClient::loadTexture(const std::string& file_path)
{
    ...
    ROS_ERROR("[Bullet] Error loading texture.");
    return -1;
}
...
bool BulletClient::changeTexture(int object_id, int joint_index, int texture_id)
{
    ...
    {
        ROS_ERROR("[Bullet] Error changing texture.");
        return false;
    }
    else
        return true;
                           ...
                           int flags)
{
	...
	if (file_name != "")
	{
		...
		{
			ROS_ERROR("[Bullet] Cannot load URDF file.");
			return -1;
		}
		return b3GetStatusBodyIndex(status_handle);
	}
	...
	{
		ROS_ERROR("[Bullet] Empty filename, method expects 1, 4 or 8 arguments.");
		return -1;
	}
}
...
bool BulletClient::resetJointState(int body_id, int joint_index, double target_value, double target_velocity)
{
    ...
    {
        ROS_ERROR("[Bullet] Joint index out-of-range.");
        return false;
    }
...
std::pair<std::array<double, 3>, std::array<double, 4>> BulletClient::getBasePositionAndOrientation(int body_id)
{
    ...
    if (status_type == CMD_ACTUAL_STATE_UPDATE_COMPLETED)
    {
        ...
        ROS_ERROR("[Bullet] getBasePositionAndOrientation failed.");
                                        ...
                                        const std::array<double, 4>& child_frame_orientation)
{
	...
	ROS_ERROR("[Bullet] createConstraint failed.");
	return -1;
}
                                                      ...
                                                      int flags)
{
	...
	if (b3CanSubmitCommand(*client_handle_))
	{
		...
		if (status_type == CMD_CAMERA_IMAGE_COMPLETED)
		{
            ...
            ROS_ERROR("[Bullet] getCameraImage failed");
	}
    else
        ROS_ERROR("[Bullet] getCameraImage can not submit the command");
...
struct b3VisualShapeInformation BulletClient::getVisualShapeData(int object_id, int flags)
{
    ...
    {
        ROS_ERROR("[Bullet] Error receiving visual shape info");
...
struct b3LinkState BulletClient::getLinkState(int body_id, int link_index, bool compute_link_velocity, bool compute_forward_kinematics)
{
    ...
    if (body_id < 0)
    {
        ROS_ERROR("[Bullet] getLinkState failed; invalid bodyUniqueId");
        return link_state_empty;
    }
    else if (link_index < 0)
    {
        ROS_ERROR("[Bullet] getLinkState failed; invalid linkIndex");
        return link_state_empty;
    }
    ...
    if (status_type != CMD_ACTUAL_STATE_UPDATE_COMPLETED)
    {
        ROS_ERROR("[Bullet] getLinkState failed.");
        return link_state_empty;
    }
...
struct b3JointInfo BulletClient::getJointInfo(int body_id, int joint_index)
{
    ...
    {
        ROS_ERROR("[Bullet] GetJointInfo failed.");
        return info;
    }
}
                                    ...
                                    int parent_link_index)
{
    ...
    {
        // ROS_ERROR("[Bullet] failed to draw debug line");
        return -1;
    }
}

long BulletClient::addUserDebugPoint(const std::array<double, 3>& xyz,
                          const std::array<double, 3>& color_rgb,
                          double point_size,
                          double life_time,
                          int pointNum,
...
{
    std::lock_guard<std::mutex> lock(mutex_);
	b3SharedMemoryCommandHandle command_handle = b3InitUserDebugDrawAddPoints3D(*client_handle_, 
                                                                              &xyz[0],
                                                                              &color_rgb[0],
                                                                              point_size, life_time, pointNum);
    ...
    {
        // ROS_ERROR("[Bullet] failed to draw debug point");
        return -1;
    }
}
                                    ...
                                    int replace_id)
{
    ...
    {
        ROS_ERROR("[Bullet] failed to draw debug text");
        return -1;
    }
}
...
bool BulletClient::removeAllUserDebugItem()
{
    std::lock_guard<std::mutex> lock(mutex_);
	b3SharedMemoryCommandHandle command_handle = b3InitUserDebugDrawRemoveAll(*client_handle_);
    b3SharedMemoryStatusHandle status_handle = b3SubmitClientCommandAndWaitStatus(*client_handle_, command_handle);
	int status_type = b3GetStatusType(status_handle);
    return (status_type != -1);
}

bool BulletClient::removeCollisionShape(int unique_id)
{
    std::lock_guard<std::mutex> lock(mutex_);
	b3SharedMemoryCommandHandle command_handle = b3InitRemoveCollisionShapeCommand(*client_handle_, unique_id);
    b3SharedMemoryStatusHandle status_handle = b3SubmitClientCommandAndWaitStatus(*client_handle_, command_handle);
    int status_type = b3GetStatusType(status_handle);
    return (status_type != -1);
}
                                                            ...
                                                            bool report_hit_number)
{
    ...
    {
        ROS_ERROR("[Bullet] Size of from_positions need to be equal to size of to_positions.");
    ...
    else
    {
        ...
        {
            ROS_ERROR("[Bullet] Number of rays exceed the maximum batch size.");
            return raycast_info_res;
        }
...
void BulletClient::performCollisionDetection()
{
    ...
    if (b3CanSubmitCommand(*client_handle_))
        ...
        ROS_WARN("[Bullet] Collisions not updated");
}
...
struct aabb_t BulletClient::getAABB(int body_id, int link_index)
{
    ...
    if (body_id < 0)
    {
        ROS_ERROR("[Bullet] getAABB failed; invalid body_id.");
        return aabb;
    }

    if (link_index < -1)
    {
        ROS_ERROR("[Bullet] getAABB failed; invalid link_index.");
        return aabb;
    }
    ...
    {
        ROS_ERROR("[Bullet] getAABB failed.");
        return aabb;
    }
...
void BulletClient::stepSimulation()
{
    ...
    if(b3CanSubmitCommand(*client_handle_))
    {
        ...
        ROS_WARN("[Bullet] simulation not performed");
}
