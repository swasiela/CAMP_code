...
BulletClient* PhysicsServers::connectPhysicsServer(BulletConnectionMothod_e method, size_t port, size_t key)
{
	...
	{
		ROS_ERROR("Exceeding maximum number of physics connections.");
		return nullptr;
	}
	...
	if (method == CONNECT_GUI)
	{
		for (size_t i = 0; i < MAX_PHYSICS_CLIENTS; i++)
		{
			...
			{
				ROS_ERROR("Only one local in-process GUI/GUI_SERVER connection allowed. Use DIRECT connection mode or start a separate GUI physics server (ExampleBrowser, App_SharedMemoryPhysics_GUI, App_SharedMemoryPhysics_VR) and connect over SHARED_MEMORY, UDP or TCP instead.");
				return nullptr;
			}
		}
	}
	...
	switch (method)
	{
		...
		default:
		{
			ROS_ERROR("connectPhysicsServer unexpected argument");
			return nullptr;
		}
	};
	...
	if (sm)
	{
		...
		if (b3CanSubmitCommand(sm))
		{
			...
			if (free_index >= 0)
			{
				...
				if (method != CONNECT_GRAPHICS_SERVER && method != CONNECT_GRAPHICS_SERVER_MAIN_THREAD)
				{
					...
					{
						ROS_ERROR("Connection terminated, couldn't get body info");
					...
					{
						ROS_ERROR("Connection terminated, couldn't get user data");
						b3DisconnectSharedMemory(sm);
						sm = 0;
						clients_handles[free_index] = 0;
						clients_method[free_index] = 0;
		...
		{
			b3DisconnectSharedMemory(sm);
			ROS_ERROR("Can not submit command to bullet");
			return nullptr;
		}
	}
	else
	{
		ROS_ERROR("Client creation failed");
		return nullptr;
	}
...
bool PhysicsServers::disconnectPhysicsServer(size_t physics_client_id)
{
	...
	if (sm == 0)
	{
		ROS_ERROR("Not connected to physics server.");
		return false;
	}
