...
bool Stomp::solve(const Eigen::MatrixXd& initial_parameters, Eigen::MatrixXd& parameters_optimized)
{
  ...
  if (!computeOptimizedCost(start_))
  {
  ...
  task_->nb_iter_ = 0;
  // Start planning time
  task_->start_time_ = std::chrono::steady_clock::now();

  while (!task_->stoppingCriterion() && runSingleIteration(start_))
  {
    CONSOLE_BRIDGE_logDebug("STOMP completed iteration %i with cost %f", current_iteration_, current_lowest_cost_);
    current_iteration_++;
    task_->nb_iter_ = current_iteration_;
  }
...
bool Stomp::runSingleIteration(std::chrono::time_point<std::chrono::high_resolution_clock> start_)
{
  if (!proceed_)
  {
    return false;
  ...
  bool proceed = generateNoisyRollouts() && computeNoisyRolloutsCosts() && filterNoisyRollouts() &&
                 computeProbabilities() && updateParameters() && computeOptimizedCost(start_);
...
bool Stomp::computeOptimizedCost(std::chrono::time_point<std::chrono::high_resolution_clock> start_)
{
  // control costs
  parameters_total_cost_ = 0;
  if (config_.control_cost_weight > MIN_CONTROL_COST_WEIGHT)
  ...
  if (current_lowest_cost_ > parameters_total_cost_)
  {
    ...
    std::vector<ompl::base::State*> stomp_opt = task_->toOMPL(parameters_optimized_);
    std::chrono::duration<double> elapsed = std::chrono::high_resolution_clock::now() - start_;
    task_->exportShct(stomp_opt, current_lowest_cost_, elapsed.count());
    std::cout << "Exported optimized trajectory with cost : " << current_lowest_cost_ << std::endl;
    task_->costsWindow_.push_back(current_lowest_cost_);
  }
  else
  {
